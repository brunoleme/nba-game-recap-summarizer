...
def test_dataset_initialization() -> None:
    dataset = NBARecapDataModule(model_name="meta-llama/Llama-3.2-1B-Instruct", source_data_path=source_data_path, env_folder=env_folder, preprocessed_input_data_folder=preprocessed_input_data_folder, batch_size=2, max_length=128, )
...
def test_preprocess_function() -> None:
    dataset_preprocessing = NBARecapDataPreprocessingModule(model_name="meta-llama/Llama-3.2-1B-Instruct", source_data_folder=source_data_folder, source_data_path=source_data_path, preprocessed_output_data_folder=preprocessed_output_data_folder, env_folder=env_folder)
    examples = {
...
def test_data_splitting_with_mocker(mocker) -> None:
    ...
    module = NBARecapDataModule(model_name="meta-llama/Llama-3.2-1B-Instruct", source_data_path=source_data_path, env_folder=env_folder, preprocessed_input_data_folder=preprocessed_input_data_folder, train_samples=10, val_samples=2, test_samples=2)
    module.prepare_data()
    module.setup()
...
def test_dataloader_creation_with_mock(mocker) -> None:
    ...
    module = NBARecapDataModule(model_name="meta-llama/Llama-3.2-1B-Instruct", source_data_path=source_data_path, env_folder=env_folder, preprocessed_input_data_folder=preprocessed_input_data_folder, train_samples=5)
    module.setup()
