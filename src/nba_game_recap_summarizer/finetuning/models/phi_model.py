...
class PhiRecapSummarizationModel(BaseRecapSummarizationModel):
    """
    Phi-3.5-mini model for NBA game recap summarization.
    ...
    def __init__(self, model_name: str = "microsoft/Phi-3.5-mini-instruct", model=None, tokenizer=None, **kwargs):
        # IMPORTANT: pass model_type="phi" so Base can set flags/PEFT correctly
        if "model_type" not in kwargs:
            kwargs["model_type"] = "phi"
    ...
    def _initialize_model(
        ...
        **kwargs,
        ...
        # Phi tokenizers often lack pad_token; set to eos for batching
        ...
        try:
            if use_quantization and self.quantization_config:
                logger.info(f"Initializing Phi-3.5-mini with: {name}, quantization: {self.quantization_type}")
                ...
                logger.info(f"Initializing Phi-3.5-mini with: {name}, no quantization")
                model = AutoModelForCausalLM.from_pretrained(
                    name,
        ...
        # Apply PEFT if configured (Base sets proper CAUSAL_LM + target_modules for Phi)
    ...
    def summarize_recap(self, game_recap: str, max_length: Optional[int] = None) -> str:
        ...
        Uses Phi-3.5-mini's chat format for better performance.
        """
        if max_length is None:
            max_length = 2048

        logger.info("Generating game recap summary (Phi-3.5-mini)")
        logger.debug(f"Input game_recap length: {len(game_recap)}")

        # Use Phi-3.5-mini's chat format for better performance
        messages = [
            {
                "role": "system", 
                "content": "You are an NBA Analyst. Summarize the following NBA game recap into a recap synthesis."
            },
            {
                "role": "user", 
                "content": f"### NBA Game Recap ###\n{game_recap}\n\n### Recap Summary ###"
            }
        ]

        # Format messages for Phi-3.5-mini
        ...
        enc = self.tokenizer(formatted_prompt, return_tensors="pt", truncation=True, max_length=self.tokenizer.model_max_length)
    ...
    def summarize_recaps(self, dataloader: DataLoader, max_length: Optional[int] = None) -> List[str]:
        ...
        logger.info("Generating recap summaries for batch (Phi-3.5-mini)")
        results: List[str] = []
        self.model.eval()
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                try:
                    ...
                    if "input_ids" in batch:
                        ...
                        # Format each game recap using Phi-3.5-mini's chat format
                        formatted_prompts = []
                        for c in convs:
                            messages = [
                                {
            ...
            if all_inputs:
                ...
                for batch_idx, (inputs, prompt_lengths) in enumerate(zip(all_inputs, all_prompt_lengths)):
                    try:
                        ...
                        max_new_tokens = min(max_length, 300)  # Cap at 300 tokens
