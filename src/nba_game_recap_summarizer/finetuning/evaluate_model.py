    ...
    with wandb.init(project=f"{cfg.project_name}-evaluation-{env_folder}", name=f"{cfg.model.name}-{cfg.model.peft_method}", tags=[f"pipeline:{pipeline_run_id}"]) as run:
        ...
        if os.path.exists(hf_model_path) and os.path.exists(os.path.join(hf_model_path, "config.json")):
            ...
            try:
                ...
                logger.warning(f"Failed to load Hugging Face model: {e}, falling back to checkpoint")
            ...
            # Fallback to checkpoint loading
            logger.info(f"Hugging Face model not found at {hf_model_path}, using checkpoint")
            model_ckpt = f"{cfg.evaluation.model_artifact_dir}/{pipeline_run_id}/checkpoints/best_model.ckpt"
            model_name = cfg.model.name
            model_type = cfg.model.type
            peft_method = cfg.model.peft_method
