import argparse
from sagemaker_pipeline_dpo import create_pipeline
...
def parse_args():
    ...
    parser.add_argument("--huggingfacehub-api-token", required=True, help="HuggingFace Hub API Token")
    ...
    parser.add_argument("--project-config", help="Project configuration to use (e.g., config.dpo.staging)")
    parser.add_argument("--base-model-path", required=True, help="S3 path to the base HF model directory (hf_model_merged)")
...
def main():
    ...
    execution = pipeline.start(
        parameters={
            ...
            "ProjectConfig": args.project_config if args.project_config else "config.dpo.staging",
            "WandbApiKey": args.wandb_api_key,
            "OpenAIApiKey": args.openai_api_key,
            "HFToken": args.hf_token,
            "HuggingFaceHubApiToken": args.huggingfacehub_api_token,
